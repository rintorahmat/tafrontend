...
function handleFileUpload(event) {
    ...
    fetch('https://0.0.0.0:8000/upload', {
        method: 'POST',
        body: formData,
    })
...
function startPreprocessing() {
    ...
    fetch(`https://0.0.0.0:8000/process/${fileId}`, {
        method: 'GET',
    })
    .then(response => response.json())
    .then(data => {
...
function splitData() {
    ...
    fetch(`https://0.0.0.0:8000/splitdata/${fileId}`, {
        method: 'POST',
        headers: {
...
function startklasification() {
    ...
    fetch(`https://0.0.0.0:8000/klasifikasi/?file_id=${hasilpreId}&test_size=${testSize}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
...
function downloadData() {
    ...
    link.href = `https://0.0.0.0:8000/download_preprocessed/${processedFileId}`;
    link.click();
}

fetch("https://0.0.0.0:8000")
